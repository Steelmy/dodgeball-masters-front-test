...
// Utils
import { GAME_STATES, EVENTS, PLAYER, DEFLECTION } from "./utils/Constants.js";
...
export class Game {
  ...
  startGame(data) {
    ...
    if (hasRemotePlayers) {
      ...
      // Clients only check deflections, NOT hits (host is authoritative for hits)
      if (!NetworkManager.isHost) {
        this.collisionSystem.isClientOnly = true;
      } else {
        this.collisionSystem.isClientOnly = false;
      }
  ...
  updateGameplay(deltaTime) {
    ...
    // Collision detection:
    // - Single player or host: full collision (hits + deflections)
    // - Client: deflections only (isClientOnly is set in startGame)
    this.collisionSystem.update();

    // Client deflect: send enriched attempt to host when local deflect succeeds
    ...
    // Network update (Send local player state) — 60Hz tick rate
    this.networkUpdateTimer += deltaTime;
    if (this.networkUpdateTimer > 0.016) {
      // ~60 times per second
  ...
  initNetwork() {
    ...
    // Missile synchronization — smooth interpolation instead of hard snap
    NetworkManager.on("onMissileUpdate", (data) => {
      // Client accepts authoritative state from host
      if (!NetworkManager.isHost && this.missile.isActive) {
        // Set network target for smooth interpolation (reconcileNetwork in Missile.update)
        this.missile.networkTargetPosition.set(data.x, data.y, data.z);
        this.missile.hasNetworkTarget = true;

        // Always sync velocity and speed for local simulation accuracy
        ...
        if (
          ...
          if (target) {
            ...
            // Host confirmed a target change — clear local deflect prediction if any
            this.missile.localDeflectPending = false;
          }
        }
      }
    });
    ...
    // Uses lag compensation: rewinds missile position based on client latency
    NetworkManager.on("onDeflectAttempt", (data) => {
      ...
      // Lag compensation: estimate where the missile WAS when the client acted
      const rPos = remotePlayer.position;
      const missilePos = this.missile.getPosition();

      // Calculate latency and rewind missile position
      const latencyMs = data.clientTime ? Date.now() - data.clientTime : 0;
      const rewindSec = Math.min(latencyMs / 1000, 0.3); // Cap at 300ms compensation
      const rewindPos = missilePos
        .clone()
        .sub(this.missile.velocity.clone().multiplyScalar(rewindSec));

      const compensatedDist = rPos.distanceTo(rewindPos);

      // Use DEFLECTION.RANGE for a more generous and consistent check
      if (compensatedDist < DEFLECTION.RANGE) {
